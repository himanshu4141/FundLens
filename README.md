...
- **Import portfolio** — 4-step friendly onboarding wizard (Welcome → Identity → Import → Done) that explains each step and links out to CAMS / KFintech via in-app browser for users who don't already have a CAS PDF. Identity step validates PAN format including the 4th-character entity code (P / C / H / F / A / T / B / L / J / G), accepts DOB in DD-MM-YYYY (Indian convention), is write-once after save with a Settings → Account "Request correction" link as the support escape hatch. Upload pipeline supports CAMS / KFintech / MFCentral with PAN password and CDSL / NSDL with PAN+DOB; second uploads are additive (duplicate transactions skipped, only new ones inserted). Auto-refresh uses Resend Inbound (`cas-<token>@foliolens.in` in prod, `cas-dev-<token>@foliolens.in` in dev) with Gmail / Outlook setup guidance and a completion checklist.
